 ...
 * @date 2020/12/07
 */
@FeignClient(value = "mall4j-search",contextId = "searchSpu")
public interface SearchSpuFeignClient {
    ...
    ServerResponse<EsPageVO<ProductSearchVO>> search(@RequestParam("pageDTO") EsPageDTO pageDTO, @RequestParam("productSearchDTO") ProductSearchDTO productSearchDTO);
    /**
     * 根据spuId列表， 获取spu列表信息
     * @param spuIds 商品id列表
     * @return 商品列表
    ...
    ServerResponse<EsPageVO<ProductSearchVO>> spuPage(@RequestParam("pageNum") Integer pageNum, @RequestParam("pageSize") Integer pageSize, @RequestParam("shopId") Long shopId);
    ...
    ServerResponse<List<SpuSearchVO>> limitSizeListByShopIds(@RequestParam("shopIds") List<Long> shopIds, @RequestParam("size") Integer size);
